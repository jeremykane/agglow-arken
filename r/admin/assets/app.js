...
app.on("/settings", function(req, res, next) {
  try {
    ...
    form.append('<div class="group homepage-redirect" style="display: ' + (homepage ? "none" : "block") + ';"> <label>Home Redirect</label> <input tooltip="Linkity will redirect people to this URL when they visit the homepage, since you\'ve opted to hide the homepage." class="input" type="text" name="homepage_redirect" placeholder="domain.com" value="' + safeInput(homepage_redirect) + '" spellcheck="false"> </div>')
...
/*
Handles showing the homepage redirect input if hiding homepage is selected.
*/
$(document).on("change", "input[name='homepage']", function(e) {
  if($(this).get(0).checked) {
    $(".homepage-redirect").slideUp(250)
  }
  else {
    $(".homepage-redirect").slideDown(250)
  }
})
...
$(document).on("submit", ".settings", function(e) {
  ...
  var form = $(this)
  form.parents(".card").addClass("loading")
  request("POST", "/settings", {name: form.find("input[name='name']").val(), homepage: form.find("input[name='homepage']").get(0).checked, homepage_redirect: form.find("input[name='homepage_redirect']").val(), domain: form.find("input[name='domain']").val(), path: form.find("input[name='path']").val()})
