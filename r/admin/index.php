...
try {
  ...
  header("Location: ".$linkity->admin->base."/sign-in.php", true, 302);
  exit;
}
  ...
  try {
    ...
    if($uri == "/admin/api/home" && $method == "GET") {
      ...
      foreach ($links["data"] as $i => $link) {
        ...
        if($linkity->path) { $links["data"][$i]["domain"] .= "/".$linkity->path; }
      }
      echo json_encode($links);
    }
    else if($uri == "/admin/api/links" && $method == "POST") {
      $link = $linkity->createLink($_POST["url"], $_POST["shortname"], $_POST["user"]);
      if($linkity->path) { $link["domain"] .= "/".$linkity->path; }
      ...
      $link = $linkity->updateLink($_GET["id"], $_POST);
      if($linkity->path) { $link["domain"] .= "/".$linkity->path; }
      ...
      if($_POST["homepage"] == "true") { $update["homepage"] = true; } else { $update["homepage"] = false; }
      if($_POST["homepage_redirect"]) { $update["homepage_redirect"] = $_POST["homepage_redirect"]; }
...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    <footer class="footer">
      <ul class="menu">
        <li class="copyright">With <i class="icon fas fa-heart"></i> from <a href="https://fhcreative.id" target="_blank">FH Creative Indonesia</a> </li>
      </ul>
    </footer>
    ...
    <aside class="slideover" id="new-link">
      ...
      <form class="link-wrapper">
        ...
        <div class="shortname">
          <?php echo $linkity->domain; ?><?php if($linkity->path) { echo "/".$linkity->path; } ?>/<input type="text" name="shortname" placeholder="shortname" spellcheck="false">
        </div>
    ...
    var homepage_redirect = "<?php if(defined("HOMEPAGE_REDIRECT")) { echo htmlspecialchars(HOMEPAGE_REDIRECT); } ?>";
