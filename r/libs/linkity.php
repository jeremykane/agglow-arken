...
class Linkity {
  ...
  function __construct() {
    ...
    //Get the homepage redirect.
    if(defined("HOMEPAGE_REDIRECT")) { $this->homepage_redirect = HOMEPAGE_REDIRECT; }
    else { $this->homepage_redirect = $this->admin->base; }
  ...
  */
  function config($new = []) {
    ...
    $homepageRedirectDefinition = "";
    if($new["homepage_redirect"]) {
      $new["homepage_redirect"] = preg_replace("/^(?!https?:\/\/)/", "http://", $new["homepage_redirect"]);
      if(!filter_var($new["homepage_redirect"], FILTER_VALIDATE_URL)) { throw new Exception("Please enter a valid URL as the homepage redirect."); }

      $homepageRedirectDefinition = 'define("HOMEPAGE_REDIRECT", "'.htmlspecialchars($new["homepage_redirect"]).'"); //This is the URL to which the script redirects when the homepage is turned off.';
    }
...
+ HOMEPAGE_REDIRECT - URL. This is the URL which the script redirects to if the homepage is turned off.
...
function randomShortname($min = 3, $max = 7) {
  if(function_exists("random_init")) {
    $length = rand_int($min, $max);
  }
  else {
    $length = rand($min, $max);
  }
...
function getHTTP() {
  ...
  else if(isset($_SERVER["HTTP_CF_VISITOR"]) && json_decode($_SERVER["HTTP_CF_VISITOR"])->scheme == "https") {
    return "https";
  }
